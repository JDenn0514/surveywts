...
test_that("print(survey_replicate) bootstrap snapshot", {
  ...
  meta@weighting_history[[1]]$timestamp <- as.POSIXct(
    "2025-01-15 10:00:00",
    tz = "UTC"
  )
...
.pin_history_ts <- function(
  obj,
  ts = as.POSIXct("2025-01-15 10:00:00", tz = "UTC")
) {
  meta <- obj@metadata
...
test_that("print(survey_nonprob) formats calibrate_linear history step", {
  ...
  cal <- suppressWarnings(calibrate_linear(
    nps,
    targets = targets,
    type = "prop"
  ))
...
test_that("print(survey_nonprob) formats calibrate_logit history step", {
  ...
  cal <- suppressWarnings(calibrate_logit(
    nps,
    targets = targets,
    type = "prop"
  ))
