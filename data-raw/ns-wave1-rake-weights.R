...
df$ns_hispanic <- factor(
  ifelse(
    df$hispanic == 1L,
    "Not Hispanic",
    ifelse(df$hispanic == 2L, "Mexican", "Other Hispanic")
  ),
...
df$ns_race <- factor(
  ifelse(
    df$race_ethnicity == 1L,
    "White",
    ifelse(
      df$race_ethnicity == 2L,
      "Black",
      ifelse(
        df$race_ethnicity %in% 4L:14L,
        "Asian/Pacific",
        ifelse(df$race_ethnicity %in% c(3L, 15L), "Other", NA_character_)
      )
    )
  ),
        ...
        ifelse(
          ...
          ifelse(
            ...
            ifelse(
              df$age %in% 60L:69L,
              "60-69",
              ifelse(df$age >= 70L, "70+", NA_character_)
            )
          )
        )
      )
...
df$ns_language <- factor(
  ifelse(
    df$language == 3L,
    "English only",
    ifelse(df$language == 1L, "Spanish", "Other")
  ),
...
df$ns_income <- factor(
  ifelse(
    is.na(df$household_income),
    "No answer",
    ifelse(
      df$household_income %in% 1L:2L,
      "<$20k",
      ifelse(
        df$household_income %in% 3L:5L,
        "$20-35k",
        ifelse(
          df$household_income %in% 6L:8L,
          "$35-50k",
          ifelse(
            df$household_income %in% 9L:11L,
            "$50-65k",
            ifelse(
              df$household_income %in% 12L:14L,
              "$65-80k",
              ifelse(
                df$household_income %in% 15L:18L,
                "$80-100k",
                ifelse(
                  df$household_income == 19L,
                  "$100-125k",
                  ifelse(
                    df$household_income %in% 20L:22L,
                    "$125-200k",
                    ifelse(
                      df$household_income %in% 23L:24L,
                      "≥$200k",
                      NA_character_
                    )
                  )
...
df$ns_vote_2016 <- factor(
  ifelse(
    ...
    ifelse(
      df$vote_2016 == 2L,
      "Clinton",
      ifelse(df$vote_2016 %in% 3L:5L, "Other", "No vote")
    )
  ),
...
cat(sprintf(
  "%-20s %8.4f %8.4f\n",
  "CV",
  sd(pub_wt) / mean(pub_wt),
  sd(rake_wt) / mean(rake_wt)
))
