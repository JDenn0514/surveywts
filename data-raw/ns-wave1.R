...
ns_wave1$ns_hispanic <- factor(
  ifelse(ns_wave1$hispanic == 1L, "Not Hispanic",
  ifelse(ns_wave1$hispanic == 2L, "Mexican",
                                  "Other Hispanic")),
...
ns_wave1$ns_race <- factor(
  ifelse(ns_wave1$race_ethnicity == 1L,           "White",
  ifelse(ns_wave1$race_ethnicity == 2L,           "Black",
  ifelse(ns_wave1$race_ethnicity %in% 4L:14L,     "Asian/Pacific",
  ifelse(ns_wave1$race_ethnicity %in% c(3L, 15L), "Other",
  NA_character_)))),
...
ns_wave1$ns_age <- factor(
  ifelse(ns_wave1$age %in% 18L:23L, "18-23",
  ifelse(ns_wave1$age %in% 24L:29L, "24-29",
  ifelse(ns_wave1$age %in% 30L:39L, "30-39",
  ifelse(ns_wave1$age %in% 40L:49L, "40-49",
  ifelse(ns_wave1$age %in% 50L:59L, "50-59",
  ifelse(ns_wave1$age %in% 60L:69L, "60-69",
  ifelse(ns_wave1$age >= 70L,        "70+",
  NA_character_))))))),
...
ns_wave1$ns_language <- factor(
  ifelse(ns_wave1$language == 3L, "English only",
  ifelse(ns_wave1$language == 1L, "Spanish",
                                  "Other")),
...
ns_wave1$ns_income <- factor(
  ifelse(is.na(ns_wave1$household_income),              "No answer",
  ifelse(ns_wave1$household_income %in% 1L:2L,          "<$20k",
  ifelse(ns_wave1$household_income %in% 3L:5L,          "$20-35k",
  ifelse(ns_wave1$household_income %in% 6L:8L,          "$35-50k",
  ifelse(ns_wave1$household_income %in% 9L:11L,         "$50-65k",
  ifelse(ns_wave1$household_income %in% 12L:14L,        "$65-80k",
  ifelse(ns_wave1$household_income %in% 15L:18L,        "$80-100k",
  ifelse(ns_wave1$household_income == 19L,              "$100-125k",
  ifelse(ns_wave1$household_income %in% 20L:22L,        "$125-200k",
  ifelse(ns_wave1$household_income %in% 23L:24L,        "≥$200k",
  NA_character_)))))))))),
...
ns_wave1$ns_vote_2016 <- factor(
  ifelse(ns_wave1$vote_2016 == 1L, "Trump",
  ifelse(ns_wave1$vote_2016 == 2L, "Clinton",
  ifelse(ns_wave1$vote_2016 %in% 3L:5L, "Other",
                                         "No vote"))),
...
.ns_targets <- list(
  ...
  ns_region = c(
    "Northeast" = 0.176, "Midwest" = 0.209,
    "South" = 0.378, "West" = 0.237
  ),
  ns_hispanic = c(
    "Not Hispanic" = 0.839, "Mexican" = 0.097, "Other Hispanic" = 0.064
  ),
  ns_race = c(
    "White" = 0.742, "Black" = 0.120,
    "Asian/Pacific" = 0.068, "Other" = 0.070
  ),
  ns_age = c(
    "18-23" = 0.095, "24-29" = 0.109, "30-39" = 0.174,
    "40-49" = 0.164, "50-59" = 0.174, "60-69" = 0.150,
    "70+"   = 0.134  # 0.133 in table; +0.001 to correct rounding to 1.0
  ),
  ns_language = c(
    "English only" = 0.783, "Spanish" = 0.129, "Other" = 0.088
  ),
  ...
  ns_vote_2016 = c(
    "Trump" = 0.272, "Clinton" = 0.284,
    "Other" = 0.033, "No vote" = 0.411  # 0.410 in table; +0.001 for rounding
  )
)
