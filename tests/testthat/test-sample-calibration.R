...
test_that("calibrate_to_survey() history has weight_stats with correct 11-key structure", {
  ...
  last <- result@metadata@weighting_history[[length(
...
test_that("calibrate_to_survey() does not error when primary has more replicates than control", {
  ...
  taylor_p <- surveycore::survey_taylor(
    df_p,
    variables = list(weights = "base_weight")
  )
...
test_that("calibrate_to_survey() accepts survey_nonprob primary + survey_replicate control", {
  skip_if_not_installed("svrep")
...
test_that("calibrate_to_survey() accepts survey_replicate primary + survey_nonprob control", {
  skip_if_not_installed("svrep")
  primary <- make_replicate_design(n = 200L, seed = 1L)
...
test_that("calibrate_to_survey() accepts both survey_nonprob with replicate weights", {
  skip_if_not_installed("svrep")
  primary <- make_nonprob_replicate_design(n = 200L, seed = 1L)
  control <- make_nonprob_replicate_design(n = 200L, seed = 2L)
...
test_that("calibrate_to_survey() nonprob history grows by exactly 1 entry per call", {
  skip_if_not_installed("svrep")
  primary <- make_nonprob_replicate_design(n = 200L, seed = 1L)
  control <- make_replicate_design(n = 200L, seed = 2L)
...
test_that("calibrate_to_estimate() accepts survey_nonprob with replicate weights", {
  skip_if_not_installed("svrep")
...
test_that("calibrate_to_estimate() nonprob history grows by exactly 1 entry per call", {
  skip_if_not_installed("svrep")
  design <- make_nonprob_replicate_design(n = 200L, seed = 1L)
  targets <- list(sex = c("F" = 110, "M" = 90))
  vcov_est <- diag(c(100, 100))
...
test_that("calibrate_to_survey() fires surveywts_error_primary_no_repweights", {
  skip_if_not_installed("svrep")
...
test_that("calibrate_to_survey() fires surveywts_error_control_no_repweights", {
  skip_if_not_installed("svrep")
...
test_that("calibrate_to_estimate() fires surveywts_error_design_no_repweights", {
  skip_if_not_installed("svrep")
...
test_that("calibrate_to_survey() primary class check fires before control (nonprob order)", {
  skip_if_not_installed("svrep")
  df <- make_surveywts_data(n = 100L, seed = 1L)
  not_replicate <- surveycore::survey_taylor(
    data = df,
...
test_that("calibrate_to_survey() primary_no_repweights fires before control check", {
  skip_if_not_installed("svrep")
  primary_no_rep <- make_nonprob_no_repweights(n = 100L, seed = 1L)
  df <- make_surveywts_data(n = 100L, seed = 2L)
  not_replicate <- surveycore::survey_taylor(
...
test_that("calibrate_to_estimate() design_no_repweights fires before targets check", {
  skip_if_not_installed("svrep")
  design_no_rep <- make_nonprob_no_repweights(n = 100L, seed = 1L)
...
test_that("calibrate_to_survey() still returns survey_replicate for replicate+replicate", {
  skip_if_not_installed("svrep")
  primary <- make_replicate_design(n = 200L, seed = 1L)
  control <- make_replicate_design(n = 200L, seed = 2L)
...
test_that("calibrate_to_survey() _not_replicate still fires for data.frame primary", {
  skip_if_not_installed("svrep")
  df <- make_surveywts_data(n = 100L, seed = 1L)
  control <- make_replicate_design(n = 100L, seed = 2L)
...
test_that("calibrate_to_survey() _not_replicate still fires for survey_taylor control", {
  skip_if_not_installed("svrep")
  primary <- make_nonprob_replicate_design(n = 100L, seed = 1L)
  df <- make_surveywts_data(n = 100L, seed = 2L)
  taylor <- surveycore::survey_taylor(
...
test_that("calibrate_to_estimate() _not_replicate still fires for data.frame design", {
  skip_if_not_installed("svrep")
  df <- make_surveywts_data(n = 100L, seed = 1L)
  targets <- list(sex = c("F" = 60, "M" = 40))
  vcov_est <- diag(c(100, 100))
...
test_that("calibrate_to_survey() accepts type = 'prop' with targets = NULL (no error)", {
  skip_if_not_installed("svrep")
  primary <- make_replicate_design(n = 100L, seed = 1L)
  control <- make_replicate_design(n = 100L, seed = 2L)
...
test_that("calibrate_to_survey() accepts algorithm = 'nr' with method = 'linear' (no error)", {
  skip_if_not_installed("svrep")
  primary <- make_replicate_design(n = 100L, seed = 1L)
  control <- make_replicate_design(n = 100L, seed = 2L)
...
test_that("calibrate_to_survey() fires scale_not_found when primary@variables$scale is NULL (targets = NULL)", {
  primary <- make_replicate_design(n = 100L, seed = 1L)
  control <- make_replicate_design(n = 100L, seed = 2L)
...
test_that("calibrate_to_survey() fires scale_not_found when primary@variables$scale is NULL (targets non-NULL)", {
  primary <- make_replicate_design(n = 100L, seed = 1L)
  control <- make_replicate_design(n = 100L, seed = 2L)
...
test_that("calibrate_to_survey() fires scale_not_found when control@variables$scale is NULL (targets non-NULL)", {
  primary <- make_replicate_design(n = 100L, seed = 1L)
  control <- make_replicate_design(n = 100L, seed = 2L)
...
test_that("calibrate_to_survey() fires scale_not_found when control@variables$scale is NULL (targets = NULL)", {
  primary <- make_replicate_design(n = 100L, seed = 1L)
  control <- make_replicate_design(n = 100L, seed = 2L)
...
test_that("calibrate_to_survey() fires control_level_missing when a level is absent from control (targets = NULL)", {
  skip_if_not_installed("svrep")
  primary <- make_replicate_design(n = 100L, seed = 1L)
...
test_that("calibrate_to_survey() fires control_level_missing when a level is absent from control (targets non-NULL)", {
  skip_if_not_installed("svrep")
  primary <- make_replicate_design(n = 100L, seed = 1L)
...
test_that("calibrate_to_survey() fires targets_not_named_list for unnamed element", {
  skip_if_not_installed("svrep")
  primary <- make_replicate_design(n = 100L, seed = 1L)
  control <- make_replicate_design(n = 100L, seed = 2L)
...
test_that("calibrate_to_survey() fires targets_not_named_list for empty list", {
  skip_if_not_installed("svrep")
  primary <- make_replicate_design(n = 100L, seed = 1L)
  control <- make_replicate_design(n = 100L, seed = 2L)
...
test_that("calibrate_to_survey() fires targets_not_named_list when targets is not a list", {
  skip_if_not_installed("svrep")
  primary <- make_replicate_design(n = 100L, seed = 1L)
  control <- make_replicate_design(n = 100L, seed = 2L)
...
test_that("calibrate_to_survey() fires targets_variable_not_found for nonexistent column", {
  skip_if_not_installed("svrep")
  primary <- make_replicate_design(n = 100L, seed = 1L)
  control <- make_replicate_design(n = 100L, seed = 2L)
...
test_that("calibrate_to_survey() fires targets_element_invalid for string element", {
  skip_if_not_installed("svrep")
  primary <- make_replicate_design(n = 100L, seed = 1L)
  control <- make_replicate_design(n = 100L, seed = 2L)
...
test_that("calibrate_to_survey() fires targets_element_invalid for unnamed numeric vector", {
  skip_if_not_installed("svrep")
  primary <- make_replicate_design(n = 100L, seed = 1L)
  control <- make_replicate_design(n = 100L, seed = 2L)
...
test_that("calibrate_to_survey() fires targets_totals_invalid for count = 0", {
  skip_if_not_installed("svrep")
  primary <- make_replicate_design(n = 100L, seed = 1L)
  control <- make_replicate_design(n = 100L, seed = 2L)
...
test_that("calibrate_to_survey() fires targets_totals_invalid for count = NA", {
  skip_if_not_installed("svrep")
  primary <- make_replicate_design(n = 100L, seed = 1L)
  control <- make_replicate_design(n = 100L, seed = 2L)
...
test_that("calibrate_to_survey() fires targets_totals_invalid when prop sum != 1", {
  skip_if_not_installed("svrep")
  primary <- make_replicate_design(n = 100L, seed = 1L)
  control <- make_replicate_design(n = 100L, seed = 2L)
...
test_that("calibrate_to_survey() rejects non-replicate primary_design [regression guard]", {
  control <- make_replicate_design(n = 100L, seed = 2L)
  df <- make_surveywts_data(n = 100L, seed = 1L)
  taylor <- surveycore::survey_taylor(
    data = df,
  ...
  expect_error(
    calibrate_to_survey(taylor, control, variables = c(sex), targets = NULL),
    class = "surveywts_error_primary_not_replicate"
  )
})

test_that("calibrate_to_survey() rejects nonprob primary with no repweights [regression guard]", {
  primary_no_rep <- make_nonprob_no_repweights(n = 100L, seed = 1L)
  control <- make_replicate_design(n = 100L, seed = 2L)
...
test_that("calibrate_to_survey() rejects non-replicate control_design [regression guard]", {
  primary <- make_replicate_design(n = 100L, seed = 1L)
  df <- make_surveywts_data(n = 100L, seed = 2L)
  taylor <- surveycore::survey_taylor(
    data = df,
  ...
  expect_error(
    calibrate_to_survey(primary, taylor, variables = c(sex), targets = NULL),
    class = "surveywts_error_control_not_replicate"
  )
})

test_that("calibrate_to_survey() rejects nonprob control with no repweights [regression guard]", {
  primary <- make_nonprob_replicate_design(n = 100L, seed = 1L)
  control_no_rep <- make_nonprob_no_repweights(n = 100L, seed = 2L)
...
test_that("calibrate_to_survey() rejects non-taylor reference_design [regression guard]", {
  skip_if_not_installed("svrep")
  primary <- make_replicate_design(n = 100L, seed = 1L)
  control <- make_replicate_design(n = 100L, seed = 2L)
...
test_that("calibrate_to_survey() rejects invalid unit_scale [regression guard]", {
  skip_if_not_installed("svrep")
  primary <- make_replicate_design(n = 100L, seed = 1L)
  control <- make_replicate_design(n = 100L, seed = 2L)
...
test_that("calibrate_to_survey() rejects variables not in primary_design [regression guard]", {
  primary <- make_replicate_design(n = 100L, seed = 1L)
  control <- make_replicate_design(n = 100L, seed = 2L)
...
test_that("calibrate_to_survey() Opsomer path returns survey_replicate for replicate primary", {
...
test_that("calibrate_to_survey() Opsomer path returns survey_nonprob for nonprob primary", {
...
test_that("calibrate_to_survey() Opsomer path changes weights (targets = NULL)", {
  primary <- make_replicate_design(n = 200L, R = 50L, seed = 42L)
  control <- make_replicate_design(n = 200L, R = 50L, seed = 99L)
...
test_that("calibrate_to_survey() Opsomer path grows history by exactly 1 (targets = NULL)", {
  primary <- make_replicate_design(n = 200L, R = 50L, seed = 42L)
  control <- make_replicate_design(n = 200L, R = 50L, seed = 99L)
  n_before <- length(primary@metadata@weighting_history)
  result <- calibrate_to_survey(primary, control, variables = c(sex))
...
test_that("calibrate_to_survey() history entry has operation == 'calibrate_to_survey'", {
  primary <- make_replicate_design(n = 200L, R = 50L, seed = 42L)
  control <- make_replicate_design(n = 200L, R = 50L, seed = 99L)
  result <- calibrate_to_survey(primary, control, variables = c(sex))
...
test_that("calibrate_to_survey() history records a_constants of length R (targets = NULL)", {
  primary <- make_replicate_design(n = 200L, R = 50L, seed = 42L)
  control <- make_replicate_design(n = 200L, R = 50L, seed = 99L)
...
test_that("calibrate_to_survey() history records K == 1L when R_C <= R (targets = NULL)", {
  primary <- make_replicate_design(n = 200L, R = 50L, seed = 42L)
  control <- make_replicate_design(n = 200L, R = 50L, seed = 99L)
  result <- calibrate_to_survey(primary, control, variables = c(sex))
  hist <- result@metadata@weighting_history
...
test_that("calibrate_to_survey() history omits targets/type/fixed_variables (targets = NULL)", {
  primary <- make_replicate_design(n = 200L, R = 50L, seed = 42L)
  control <- make_replicate_design(n = 200L, R = 50L, seed = 99L)
  result <- calibrate_to_survey(primary, control, variables = c(sex))
  hist <- result@metadata@weighting_history
...
test_that("calibrate_to_survey() Opsomer linear path matches svrep oracle within 1e-8", {
  skip_if_not_installed("svrep")
  primary <- make_replicate_design(n = 200L, R = 50L, seed = 42L)
  control <- make_replicate_design(n = 200L, R = 50L, seed = 99L)
  set.seed(1L)
...
test_that("calibrate_to_survey() Opsomer rake path satisfies full-sample control totals", {
  primary <- make_replicate_design(n = 200L, R = 50L, seed = 42L)
  control <- make_replicate_design(n = 200L, R = 50L, seed = 99L)
  result <- calibrate_to_survey(primary, control, variables = c(sex))
...
test_that("calibrate_to_survey() Opsomer fixed-margin returns survey_replicate", {
  primary <- make_replicate_design(n = 200L, R = 50L, seed = 42L)
  control <- make_replicate_design(n = 200L, R = 50L, seed = 99L)
...
test_that("calibrate_to_survey() Opsomer fixed-margin returns survey_nonprob for nonprob primary", {
  primary <- make_nonprob_replicate_design(n = 200L, R = 30L, seed = 1L)
  control <- make_replicate_design(n = 200L, R = 30L, seed = 99L)
...
test_that("calibrate_to_survey() history records fixed_variables from targets", {
  primary <- make_replicate_design(n = 200L, R = 50L, seed = 42L)
  control <- make_replicate_design(n = 200L, R = 50L, seed = 99L)
  sex_prop <- c("M" = 0.49, "F" = 0.51)
  result <- calibrate_to_survey(
...
test_that("calibrate_to_survey() history records K == 1L when R_C <= R (targets non-NULL)", {
  primary <- make_replicate_design(n = 200L, R = 50L, seed = 42L)
  control <- make_replicate_design(n = 200L, R = 50L, seed = 99L)
  sex_prop <- c("M" = 0.49, "F" = 0.51)
  result <- calibrate_to_survey(
...
test_that("calibrate_to_survey() history records K == 2L when R_C > R", {
...
test_that("calibrate_to_survey() history records type == 'count' when type = 'count'", {
  primary <- make_replicate_design(n = 200L, R = 50L, seed = 42L)
  control <- make_replicate_design(n = 200L, R = 50L, seed = 99L)
...
test_that("calibrate_to_survey() type='prop' accepted with non-NULL targets", {
  primary <- make_replicate_design(n = 200L, R = 50L, seed = 42L)
  control <- make_replicate_design(n = 200L, R = 50L, seed = 99L)
  sex_prop <- c("M" = 0.49, "F" = 0.51)
  result <- calibrate_to_survey(
...
test_that("calibrate_to_survey() fixed margin constraint satisfied within 1e-6", {
  primary <- make_replicate_design(n = 200L, R = 50L, seed = 42L)
  control <- make_replicate_design(n = 200L, R = 50L, seed = 99L)
  N <- sum(primary@data[[primary@variables$weights]])
  sex_prop <- c("M" = 0.49, "F" = 0.51)
...
test_that("calibrate_to_survey() reference_design stored in history when supplied", {
  primary <- make_replicate_design(n = 200L, R = 50L, seed = 42L)
  control <- make_replicate_design(n = 200L, R = 50L, seed = 99L)
...
test_that("calibrate_to_survey() all a_r == sqrt(A_C/A) when R == R_C", {
...
test_that("calibrate_to_survey() a_r[r] == sqrt(A_C/A) for r <= R_C, 0 for r > R_C", {
...
test_that("calibrate_to_survey() K == ceiling(R_C / R) when R_C > R", {
...
test_that("calibrate_to_survey() a_r computed with A_eff = A/K when K > 1", {
...
test_that("calibrate_to_survey() satisfies full-sample random-margin constraint", {
  primary <- make_replicate_design(n = 200L, R = 50L, seed = 42L)
  control <- make_replicate_design(n = 200L, R = 50L, seed = 99L)
  result <- calibrate_to_survey(primary, control, variables = c(sex))
...
test_that("calibrate_to_survey() satisfies full-sample fixed-margin constraint", {
  primary <- make_replicate_design(n = 200L, R = 50L, seed = 42L)
  control <- make_replicate_design(n = 200L, R = 50L, seed = 99L)
  N <- sum(primary@data[[primary@variables$weights]])
  sex_prop <- c("M" = 0.49, "F" = 0.51)
...
test_that("calibrate_to_survey() type='prop' uses N from input primary weights", {
  primary <- make_replicate_design(n = 200L, R = 50L, seed = 42L)
  control <- make_replicate_design(n = 200L, R = 50L, seed = 99L)
  N <- sum(primary@data[[primary@variables$weights]])
...
test_that("calibrate_to_survey() accepts Format B (tibble) targets", {
  primary <- make_replicate_design(n = 200L, R = 50L, seed = 42L)
  control <- make_replicate_design(n = 200L, R = 50L, seed = 99L)
  N <- sum(primary@data[[primary@variables$weights]])
...
test_that("calibrate_to_survey() accepts mixed-format targets (Format A + Format B)", {
  primary <- make_replicate_design(n = 200L, R = 50L, seed = 42L)
  control <- make_replicate_design(n = 200L, R = 50L, seed = 99L)
  N <- sum(primary@data[[primary@variables$weights]])
...
test_that("calibrate_to_survey() does not call svrep::calibrate_to_sample() in any path", {
  skip_if_not_installed("svrep")
...
test_that("calibrate_to_survey() scale_not_found error is not regressed by PR 2", {
  primary <- make_replicate_design(n = 100L, R = 50L, seed = 42L)
  control <- make_replicate_design(n = 100L, R = 50L, seed = 99L)
...
test_that("calibrate_to_survey() R > R_C: output has R replicate columns", {
  df60 <- make_surveywts_data(n = 100L, seed = 42L)
  t60 <- surveycore::survey_taylor(
    data = df60,
    variables = list(weights = "base_weight")
...
test_that("calibrate_to_survey() R_C > R: output still has R replicate columns", {
  primary <- make_nonprob_replicate_design(n = 100L, R = 30L, seed = 1L)
  control <- make_replicate_design(n = 100L, R = 50L, seed = 99L)
  R <- length(primary@variables$repweights)
  result <- calibrate_to_survey(primary, control, variables = c(sex))
...
test_that("calibrate_to_survey() a_r=0 replicates produce finite weights (no Inf/NaN)", {
  df60 <- make_surveywts_data(n = 100L, seed = 42L)
  t60 <- surveycore::survey_taylor(
    data = df60,
    variables = list(weights = "base_weight")
...
test_that("calibrate_to_survey() warns for unknown control param (regression guard PR 2)", {
  primary <- make_replicate_design(n = 100L, R = 50L, seed = 42L)
  control <- make_replicate_design(n = 100L, R = 50L, seed = 99L)
...
test_that("calibrate_to_survey() warns for replicate scheme mismatch (regression guard PR 2)", {
  skip_if_not_installed("svrep")
  primary <- make_replicate_design(n = 100L, R = 50L, seed = 42L)
...
test_that("calibrate_to_survey() warns for negative full-sample weights (method=linear)", {
...
test_that("calibrate_to_survey() targets=NULL with type='prop' is identical to default", {
  primary <- make_replicate_design(n = 100L, R = 50L, seed = 42L)
  control <- make_replicate_design(n = 100L, R = 50L, seed = 99L)
  set.seed(1L)
...
test_that("calibrate_to_survey() algorithm='nr' with method='linear' does not error", {
  primary <- make_replicate_design(n = 100L, R = 50L, seed = 42L)
  control <- make_replicate_design(n = 100L, R = 50L, seed = 99L)
  expect_no_error(
    suppressWarnings(
...
test_that("calibrate_to_survey() R = R_C = 2 minimum replicates: K=1, valid result", {
...
test_that("calibrate_to_survey() negative replicate weights do not trigger warning", {
  primary <- make_replicate_design(n = 100L, R = 50L, seed = 42L)
  control <- make_replicate_design(n = 100L, R = 50L, seed = 99L)
  # Replicate weights are allowed to be negative — only full-sample negatives warn
  expect_no_warning(
    calibrate_to_survey(primary, control, variables = c(sex), method = "rake")
  )
})

test_that("calibrate_to_survey() unit_scale non-NULL with Opsomer path produces valid result", {
  primary <- make_replicate_design(n = 100L, R = 50L, seed = 42L)
  control <- make_replicate_design(n = 100L, R = 50L, seed = 99L)
