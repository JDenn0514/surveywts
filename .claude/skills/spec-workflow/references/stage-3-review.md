...
- For Tier 3 (Algorithmic) and Tier 4 (Dispatcher) functions: are the required
  `@section Algorithm` and `@references` entries called out in the spec? For
  iterative Tier 3 functions: is `@section Convergence` specified? Missing these
  is REQUIRED (not a suggestion) — the builder has no documentation contract
  without them.
