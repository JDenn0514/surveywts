...
| `surveywts_error_bounds_invalid_calibration` | `.validate_bounds()`, `calibrate_linear()`, `calibrate_logit()` | `bounds` is non-`NULL` but invalid: not length-2 numeric; for `bounds_scale = "multiplicative"`, `L >= 1` or `U <= 1`; for `bounds_scale = "absolute"`, `L <= 0` or `L >= U`; or either value is `NA` or non-finite. Also thrown by `calibrate_logit()` with `bounds_scale = "absolute"` when any base weight `d_k <= L_abs` (making the per-unit lower g-weight bound `L_vec[k] = L_abs / d_k >= 1`) or `d_k >= U_abs` (making `U_vec[k] = U_abs / d_k <= 1`), rendering the logit calfun ill-defined. |
