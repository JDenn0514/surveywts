...
| 9 — Phase B and Phase C | **PASS** | 9 blind auditors mapped all 1,102 rows: 905 MOVED, 98 DELETED (all approved), 54 PRESERVED, 43 DEDUPED (all quoted), 2 ALTERED (both the instructed directory-pointer updates, waived in `plans/ledger/phasec-waivers.tsv`), 0 LOST. The audit caught 2 real losses — the B6/B7 wrapper requirements — restored before reconciliation. Mapping in `plans/ledger/phaseb-*.tsv` |
| 10 — reachability test | **PASS, with two remediations** | Valid arm (agents dispatched in the main repo; worktree dispatches proved invalid — the Agent tool cuts worktrees from develop, so their cwd lacked `.claude/standards/`): builder 6/6, planner 3/3, reviewer 3/3, shipper 3/3 (one via Bash cat after its stale-path Read failed). Tester failed 0/3 across two wording rounds; remediated per the sharpen-then-inline ladder by dispatch-time injection — pipeline-ship and pipeline-simplified now paste the tester standards into its dispatch prompt, with a fallback-read Step 0 — then verified 3/3 on in-role validation drills, each read confirmed in the transcript. Sharpened Step 0 (first-tool-call bound, citation honesty bar) landed on all 8 definitions. `usage-profile.py --entry-fee` added; pipeline-role entry fee measured 47,881–51,559 against the 68,471/73,246 baseline — builder and tester targets met |
| 11 — confirmatory pipeline run | deferred to post-merge | Section 9.2 itself defines this as a run "after the work lands"; the branch is unmerged. Run one Tier-3 change through the pipeline after merge and write the report |
